...
int main() {
  try {
    ...
    std::optional<smart_home::DeviceRecord> saved_wifi;
    {
      ...
      require(saved_wifi->name == "Light", "Inserted record must contain name");
      require(storage.findByPhysicalId("AC:12:DD", "wifi.light") == saved_wifi,
      ...
      expectException<std::invalid_argument>([&] {
        storage.insert("AC:12:DD", "zigbee.temperature_sensor", "Duplicate TS");
      });
      require(storage.getAll().size() == 2, "All records must be in memory");
      auto bluetooth_light =
          storage.insert("AC:12:DD", "bluetooth.light", "Лампа's на кириллице");
    ...
    }
    {
      ...
      const auto restored = storage.findByPhysicalId("AC:12:DD", "wifi.light");
